...
| 12 月 | `2`（12 月自定义区间） | 同商户、同业务性质、12 月最多 **2 笔**汇总（见 §1.2） | **1~N 张**，N ≤ `invoice_limit`；同一申请多票 **BZ 相同** | 跨月 `yyyyMM-yyyyMM` | `202512-202604` |

说明：

1. **BZ = 汇总周期**，不是商户抬头里的「发票摘要」（`merchant_invoice.invoice_remark`）。
2. 财务票面样例：备注栏为 `202512-202604`（6 位年月 + 连字符，**不是** `yyyy-MM`）。
...
| 2 | `periodStartAt` 所在月第一天 | 与起始不同月时取 `periodEndAt` 所在月 | `yyyyMM-yyyyMM` 或单月 |
...
```text
period_type = 2 且 settlement_mon_end 非空且与 start 不同月
  → CONCAT(format(start,'yyyyMM'), '-', format(end,'yyyyMM'))
否则
...
| 步骤 | 操作 | 预期 |
| --- | --- | --- |
| T-BZ-11 | 12 月创建汇总，区间如 2025-12 ~ 2026-04 | `period_type=2`；`period_end_at` 为区间结束日；`settlement_mon_end` 为 2026-04-01 |
| T-BZ-12 | 申请拆 2 张票（两抬头） | 两条 detail.remark **相同**：`202512-202604` |
| T-BZ-13 | 复审通过生成 2 条 invoice | 两条 `remark` 均为 `202512-202604` |
| T-BZ-14 | 乐企报文 | 两次 `GP_FPKJ` 的 `BZ` 一致且为区间格式 |
...
| BZ 为 `202512` 而非 `202512-202604` | 汇总创建未传 `periodEndAt` 或未落库 | 查 `etcplus_recon_bill.period_end_at` |
